...
```{r}
TaylorExpansionModelExpAvg <- function(x, lag, degree, freq, forecastPeriod, stepSize){
  offset <- lag + degree
  inSample <- rep(0, length(x) - offset + forecastPeriod)
  inSample <- ts(inSample, frequency=freq, start=time(x)[offset + 1])
  derivatives <- rep(0, degree)
...
plot(a, ylim=c(0, max(pred)*1.1), xlim=c(2006.5, 2025))
lines(pred, lty=2, col='blue')

pred = TaylorExpansionModelAvg(x, lag=1, degree=10, freq=1, forecastPeriod=75, stepSize=(2.5/250))

plot(x, ylim=c(-abs(min(pred)*1.1), max(pred)*1.1), xlim=c(0, length(x)*1.25))
lines(pred, lty=2, col='blue')
```
...
for (col in cols){
  ...
  pred = TaylorExpansionModelAvg(tempTS, lag=10, degree=2, freq=4, forecastPeriod=8, stepSize=0.25)
